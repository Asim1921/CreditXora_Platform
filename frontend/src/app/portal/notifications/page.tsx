...
export default function PortalNotificationsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setItems(await api.get<NotificationItem[]>("/portal/notifications", true));
      setError(null);
    } catch {
      setError("We couldn't load your notifications. Please refresh and try again.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const markAllRead = async () => {
    setMarking(true);
    try {
      await api.post("/portal/notifications/read", undefined, true);
      ...
      <section className="card-surface p-6">
        ...
        {items.length === 0 ? (
          <EmptyState
            ...
            description="Updates about your documents, disputes and file status appear here."
          />
        ) : (
